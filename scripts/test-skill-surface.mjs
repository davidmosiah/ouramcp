...
assert.match(skill, /call oura_connection_status/);
...
const result = await run(["call", "oura_connection_status", "--json", "{}"]);
...
console.log(JSON.stringify({ ok: true, suite: "skill-surface", tool: "oura_connection_status", files: pkg.files }, null, 2));
